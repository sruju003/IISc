...
def get_data():
    ...
    if connection:
        ...
        # Get names
        distinct_names_query = "SELECT DISTINCT fname FROM responses_hardware"
        distinct_names = execute_read_query(connection, distinct_names_query)
        names = [name[0] for name in distinct_names]

        # Get people
        distinct_people_query = "SELECT DISTINCT software FROM responses_hardware"
        ...
        question_query = "SELECT DISTINCT question FROM responses_hardware ORDER BY question"
            ...
            for name in names:
                ...
                for person in people:
                    ...
                    for question in pair:
                        query = "SELECT response FROM responses_hardware WHERE fname=%s AND hardware=%s AND question=%s"
...
def home():
    ...
    return render_template('final11.html', names=names, people=people, tabs_data=tabs_data)
