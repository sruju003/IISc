...
def get_data():
    ...
    if connection:
        ...
        # Get names
        distinct_names_query = "SELECT DISTINCT fname FROM responses_software"
        distinct_names = execute_read_query(connection, distinct_names_query)
        names = [name[0] for name in distinct_names]

        # Get people
        distinct_people_query = "SELECT DISTINCT software FROM responses_software"
        ...
        question_query = "SELECT DISTINCT question FROM responses_software ORDER BY question"
            ...
            for name in names:
                ...
                for person in people:
                    ...
                    for question in pair:
                        query = "SELECT response FROM responses_software WHERE fname=%s AND software=%s AND question=%s"
